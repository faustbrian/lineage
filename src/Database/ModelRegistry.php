 ...
 * when different models use different key types (e.g., User with 'uuid', Seller with 'ulid').
 *
...
final class ModelRegistry
{
     ...
     * will throw a MorphKeyViolationException. When false, defaults to model's key name.
     */
     ...
     * Lineage::for($user)->attach($parent, 'seller');
     * Lineage::for($seller)->attach($parent, 'seller');
     *
     * // This will throw MorphKeyViolationException
     * Lineage::for($post)->attach($parent, 'seller'); // Post not in mapping
     * ```
     *
     * @param array<class-string, string> $map Model class => column name mappings
     */
    ...
    /**
     * Get the polymorphic key value for a model.
     *
     * Returns the value of the model's configured key column.
     *
     * @param Model $model The model to get the key value for
     *
     * @throws MorphKeyViolationException If enforceKeyMap is true and model has no mapping
     *
